...
The active pane heading uses reverse video. The selected task uses `>`. At this width the Tasks footer would need more than three rows, so it is hidden and `?` opens the [Help overlay](template-help-overlay.md) instead. Archive labels change contextually as specified in the [archive controls](../ui.md#archive-controls).
